...
namespace workertest
{
    ...
    public abstract class AmqConsumerService<T> : PollingService<T>
    {
        ...
        protected AmqConsumerService(ILogger<T> logger,
                                     IAmqConfiguration<T> amqConfig) : base(amqConfig, logger)
        {
        ...
        protected override async Task<bool> ExecutePollerAsync(CancellationToken stoppingToken)
        {
            ...
            {
                Logger.LogError($"Error while recieving message from queue {_amqConfig.BrokerUri} {_amqConfig.QueueName}", ex);
                TeardownAMQ();
                return true;
            }
            ...
            try
            {
                Logger.LogInformation($"Processing message {mqTextMessage.NMSMessageId} received from {_amqConfig.BrokerUri} {_amqConfig.QueueName}");
            ...
            {
                Logger.LogError($"Exception during processing of message {mqTextMessage.NMSMessageId}", ex);
        ...
        private bool SendToExceptionQueue(ITextMessage message)
        {
            try
            {
                Logger.LogError($"Message {message.NMSMessageId} being moved to the Exception Queue {_amqConfig.ExceptionQueueName}.");
            ...
            {
                Logger.LogError($"Error while sending message {message.NMSMessageId} to exception queue {_amqConfig.BrokerUri} {_amqConfig.ExceptionQueueName}", ex);
                return false;
            }
        }
        ...
        private void TeardownAMQ()
        {
            ...
            {
                Logger.LogError($"Error during AMQ cleanup of {_amqConfig.BrokerUri} {_amqConfig.QueueName}", ex);
                _session = null;
                _connection = null;
                _consumer = null;
            }
